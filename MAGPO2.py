...
class Agent:
    def __init__(self,
        ...
        self.func_action = GLCN(feature_size=self.n_representation_action,
                                graph_embedding_size=self.n_representation_action+15, link_prediction=False).to(device)
        ...
        if mini_batch == False:
            with torch.no_grad():
                ...
                if cfg.given_edge == True:
                    ...
                    node_embedding = self.func_glcn(dead_masking= dead_masking, X = agent_embedding, A = self.fully_connected_edge)
            ...
            if cfg.given_edge == True:
                ...
                node_embedding = self.func_glcn(dead_masking= dead_masking, X = agent_embedding, A = fully_connected_edges, mini_batch = mini_batch)
                #node_embedding = self.func_glcn2(dead_masking=dead_masking, X=node_embedding, A=A)
                return node_embedding
    ...
    def learn(self, cum_loss = 0):
        ...
        for i in range(self.K_epoch):
            ...
            for l in range(len(self.batch_store)):
                ...
                action_embedding = torch.concat([action_embedding[:,:6, :], action_embedding[:,6 + num_agent:, :]], dim=1)
