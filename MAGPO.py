...
class Agent:
    def __init__(self,
        ...
        self.node_representation_sum_state = NodeEmbedding(feature_size=12,
                                                           ...
                                                           n_representation_obs=self.n_representation_obs).to(
            device)  # 수정사항

        self.node_representation_agent = NodeEmbedding(feature_size=2 * self.feature_size + 5,
                                                       hidden_size=self.hidden_size_obs,
                                                       n_representation_obs=self.n_representation_obs).to(
            device)  # 수정사항
        ...
        self.action_representation = NodeEmbedding(feature_size=self.feature_size + 6,
                                                   hidden_size=self.hidden_size_action,
                                                   n_representation_obs=self.n_representation_action).to(device)  # 수정사항

        self.func_action = GLCN(feature_size=self.n_representation_action,
                                graph_embedding_size=self.n_representation_action + 15, link_prediction=False).to(
            device)
        ...
        self.network_no_attack = PolicyNoAttack(
            state_action_size=self.n_representation_obs + self.graph_embedding_comm,
            layers=self.layers).to(device)

        self.network = PPONetwork(state_size=self.graph_embedding_comm,
                                  state_action_size=self.n_representation_obs + self.graph_embedding_comm + self.n_representation_action + 15,
                                  layers=self.layers).to(device)

        self.valuenetwork = ValueNetwork(state_size=self.graph_embedding_comm + + self.n_representation_obs,
                                         state_action_size=self.graph_embedding_comm
                                                           + self.n_representation_action,
                                         layers=self.layers).to(device)
    ...
    @torch.no_grad()
    def get_td_target(self, ship_features, node_features_missile, heterogenous_edges, possible_actions, action_feature,
                      reward, done):
        obs_next, act_graph = self.get_node_representation(ship_features, node_features_missile, heterogenous_edges,
                                                           mini_batch=False)
    ...
    @torch.no_grad()
    def sample_action(self, summarized_state_feature, node_representation, action_feature, avail_action, num_agent,
                      edge_index_obs,
                      epsilon=0):
        """
        ...
        action_size = action_feature.shape[0] - 6
        action = []
        probs = []
        ...
        for a in range(6):
            edge_index_obs2[0].append(a - 6)
            edge_index_obs2[1].append(a - 6)
        edge_index_obs2 = torch.tensor(edge_index_obs2).long().to(device) + 6
        edge_index_obs2[[0, 1]] = edge_index_obs2[[1, 0]]

        action_embedding = self.func_action(X=action_embedding, A=edge_index_obs2)
        action_embedding = torch.concat([action_embedding[:6, :], action_embedding[6 + num_agent:, :]], dim=0)
        ...
        for n in range(num_agent):
            ...
            self.network_no_attack(obs_cat)
    ...
    def get_node_representation_gpo(self, node_feature, agent_feature, edge_index_obs, edge_index_comm, n_agent,
                                    dead_masking, mini_batch=False):
        if mini_batch == False:
            with torch.no_grad():
                ...
                embedding = node_embedding_obs  # torch.concat([obs_embedding, enemy_embedding], axis=0)
                ...
                if cfg.given_edge == True:
                    ...
                    node_embedding = self.func_glcn(dead_masking=dead_masking, X=agent_embedding,
                                                    A=self.fully_connected_edge)
                    # print("2", node_embedding)
            ...
            if cfg.given_edge == True:
                ...
                node_embedding = self.func_glcn(dead_masking=dead_masking, X=agent_embedding, A=fully_connected_edges,
                                                mini_batch=mini_batch)
                # node_embedding = self.func_glcn2(dead_masking=dead_masking, X=node_embedding, A=A)
                return node_embedding
    ...
    def learn(self, cum_loss=0):
        ...
        for i in range(self.K_epoch):
            ...
            for l in range(len(self.batch_store)):
                ...
                num_action = action_feature.shape[1] - 6
                ...
                action_feature = action_feature.reshape(time_step * (num_action + 6), -1).to(device)
                action_embedding = self.action_representation(action_feature)
                action_embedding = action_embedding.reshape(time_step, (num_action + 6), -1)
                # print(torch.tensor(edge_index_enemy_list[0]).shape,
                #       torch.tensor(edge_index_enemy_list[1]).shape,
                #       torch.tensor(edge_index_enemy_list[2]).shape,
                #       torch.tensor(edge_index_enemy_list[3]).shape)
                edge_index_obs2 = list()
                for e in range(len(edge_index_enemy_list)):
                    edge_index = deepcopy(edge_index_enemy_list[e])
                    for a in range(6):
                        edge_index[0].append(a - 6)
                        edge_index[1].append(a - 6)
                    new_edge_index = torch.tensor(edge_index, dtype=torch.long).to(device) + 6
                    new_edge_index[[0, 1]] = new_edge_index[[1, 0]]

                    edge_index_obs2.append(new_edge_index)

                # edge_index_obs2 = torch.tensor(edge_index_enemy_list, dtype=torch.long).to(device) + 6
                action_embedding = self.func_action(X=action_embedding, A=edge_index_obs2, mini_batch=True)
                action_embedding = torch.concat([action_embedding[:, :6, :], action_embedding[:, 6 + num_agent:, :]],
                                                dim=1)

                node_embedding2 = node_embedding2[:, :num_agent, :]
                node_embedding_next2 = torch.cat((node_embedding2, empty), dim=0)[1:, :, :]

                v_s = self.valuenetwork(node_embedding2.reshape(num_agent * time_step, -1))
                v_next = self.valuenetwork(node_embedding_next2.reshape(num_agent * time_step, -1))
                ...
                for n in range(num_agent):
                    ...
                    # print(summarized_state.shape, obs.shape, action_embedding.shape)
                ...
                if cfg.given_edge == True:
                    loss = -(pi.squeeze(1) * advantage).mean() + 0.5 * value_loss
                else:
